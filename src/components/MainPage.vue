  ...
  methods: {
    ...
    async sendMessageToBot() {
      ...
      try {
        ...
        await axios.post('https://ada2-154-161-191-11.ngrok-free.app/messaging/send', {
    ...
    async getBotResponse() {
      console.log("Receiving message from bot")
      try {
        const response = await axios.get("https://ada2-154-161-191-11.ngrok-free.app/messaging/listen");
        console.log("From Bot: " + response);

        // Add the bot's response to the conversation
        this.conversation.push({ type: 'bot', text: response || "No response received." });
      } catch (error) {
